...
describe("Route: PATCH /actions/:actionID", async () => {
  ...
  it("can return a 200 status code and the requested action", async () => {
    ...
    const newActionProperties: InitialWritableActionProperties = {
      name: `${generateUUIDv7()}.${generateUUIDv7()}.${generateUUIDv7()}`,
      displayName: TestEnvironment.generateRandomString(64),
      description: TestEnvironment.generateRandomString(255)
    }
    ...
    strictEqual(jsonResponse.name, newActionProperties.name);
    strictEqual(jsonResponse.displayName, newActionProperties.displayName);
    strictEqual(jsonResponse.description, newActionProperties.description);

    const updatedAction = await Action.getByID(action.id, slashstepServer.pool);
    strictEqual(updatedAction.name, newActionProperties.name);
    strictEqual(updatedAction.displayName, newActionProperties.displayName);
    strictEqual(updatedAction.description, newActionProperties.description); 
